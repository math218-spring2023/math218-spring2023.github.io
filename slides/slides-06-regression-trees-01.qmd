...
Simple regression trees can be implemented in `R` using the `trees` library

-   Note: the `tree()` function we will use requires all categorical variables to be coded as *factors*

-   Syntax is just as in `lm()`:
...
-   We see number of terminal notes $|T_{0}|$, the predictors that were used to build the tree, and residual mean deviance

    -   If you don't see list of predictors, then the tree used all of them

    -   Residual mean deviance: $\text{RSS}/(n - |T_{0}|)$

## tree() output {.scrollable}
...
-   We will use base `R` plots; I'm sure there are much more beautiful `ggplot` functions out there, but I'm not in the business of it today!
