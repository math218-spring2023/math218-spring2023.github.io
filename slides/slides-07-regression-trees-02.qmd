...
## Create large tree {.scrollable}

Fit a large regression tree for `abundance` using all the predictors:
...
- Maybe we think this tree is too complex and might want to prune it to improve results

- We can use `cv.tree()` to perform k-fold cross-validation in order to determine the optimal level of tree complexity

  - This function uses cost-complexity pruning, and defaults to k=10 for the k-fold CV
...
#| echo: true
#| eval: false
set.seed(3) # why am I setting a seed?
...
## Choosing pruned tree

```{r}
min_size <- cv_mites$size[which.min(cv_mites$dev)]
data.frame(error_rate = cv_mites$dev, size = cv_mites$size, k = cv_mites$k) %>%
...
- Based on output, the tree with `r min_size` terminal nodes seems to be best. 
...
- Results in a much simpler tree which we could then use for predictions!
