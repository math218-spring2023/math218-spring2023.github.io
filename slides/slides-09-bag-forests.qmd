...
-   Can obtain an overall summary of importance of each predictor using the residual sum of square (RSS):

    -   For bagged regression trees, record the total amount that RSS decreases due to splits over a given predictor, averaged over $B$
:::
...
-   Then we build a regression tree where:
    ...
    -   For node 1, we only consider a random sample of $m \approx \sqrt{5} \approx 2$ predictors:
