...
-   We will now see how we can use KNN for classification
    ...
    -   *Side note*: when people refer to KNN, they almost always refer to KNN for classification tasks
...
#| fig-height: 7
p3 <- ggplot()+
  geom_point(data = train_dat, aes(x = SubsDens, y = WatrCont, col = present), size = 3) +
...
- An issue we may encounter in KNN classification is ties
