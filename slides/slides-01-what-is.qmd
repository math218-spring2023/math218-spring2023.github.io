...
Please accept the invitation to join our GitHub organization! Let me know if you did not receive an e-mail.
...
-   We have a set of inputs or predictors $X$, and we want to predict a corresponding $Y$. Assume the true model is $Y = f(X) + \epsilon$, but don't know $f$

-   Assuming the error $\epsilon$ is 0 on average, we can obtain predictions of $Y$ as $$\hat{Y} = \hat{f}(X)$$

    -   Then, if we know the true $Y$, we can evaluate the accuracy of the prediction $\hat{Y}$
...
-   ::: important
    ...
    -   $\epsilon$ may also contain unmeasured variables that are not available to us
...
I generated some fake data and fit three models that differ in flexibility,
