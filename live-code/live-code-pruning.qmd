...
If we want to explicitly tell `tree()` to grow a larger tree, we can specify the optional argument called `control`. We set `nobs = n` (i.e. the number of training observations), and another optional control argument such as `mindev`, `minsize`, or `mincut`. `mindev = 0` . See the help file for `tree.control` for more details.
