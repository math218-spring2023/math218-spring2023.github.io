...
Billy Beane and Paul DePodesta ultimately decided to remove batting average from their model for runs scored because they had limited financial resources and wanted to find the skills that were undervalued. However, it could be the case that knowing the batting average of a player is important to explaining the runs scored, and also *predicting* the runs scored. After all, we had to make predictions for the 2002 season, so we would like a model that predicts the runs scored well. Let's examine this now to see if the team would have made better predictions of runs scored if they had kept the batting average in the model.

### Data preparation

::: {style="color: maroon"}
Split your data into a 80% train set and a 20% test set. Be sure to set a seed for reproducibility!
:::

### Model fitting

::: {style="color: maroon"}
Fit two new models called `mod_rs1_train` and `mod_rs2_train` which are the analogs of `mod_rs1` and `mod_rs2` but fit on the *train* data only.
:::

### Prediction error

::: {style="color: maroon"}
Obtain predictions of runs scored from both `mod_rs1_train` and `mod_rs2_train` for the *test* data. Then calculate and report the root mean squared error for the predictions from both of the models.
:::

*Please do not report/display/print the vectors of predictions! Just store them using an appropriate variable name.*

::: {style="color: maroon"}
Based on your results, do you think the Oakland A's should have kept batting average in their model for runs scored? Why or why not?
:::
